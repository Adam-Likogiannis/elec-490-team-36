...
# Robustness mode for capstone experiments:
# keep keyed whitening for privacy, but DO NOT use dimension-dependent keyed spatial shuffling.
# This keeps payload blocks anchored in raster order near the top-left of the image, so
# simple bottom-crop edits do not destroy header synchronization.

USE_KEYED_SPATIAL_SHUFFLE = False

HEADER_RING_COUNT = 3

# Decode-side robustness search:
# keep embedding simple/fast, and let the decoder search over likely image transforms.
ROBUST_DECODE_SEARCH = True
SEARCH_ANGLE_WIGGLE_DEG = 0.0
SEARCH_ANGLE_STEP_DEG = 1.0
...
# --- Center-ring robust block layout ---

@lru_cache(maxsize=32)
def _centerRingLayoutCache(
        height: int,
        width: int,
        block: int,
        headerBits: int,
    ...
    """
    Build and cache the deterministic center-first block layout for a given LL size.

    Returns:
      - ordered block coordinates (header-first, then outward)
      - total number of usable coordinates in that ordering
      - number of slots that lie inside the reserved header rings

    This is cached because camera images often share the same resolution, and the
    old implementation rebuilt + sorted all blocks in Python on every call.
    """
    ny = height // block
    nx = width // block
    total = ny * nx
    ...
    dy = rows.astype(np.float32) - np.float32(centerRow)
    dx = cols.astype(np.float32) - np.float32(centerCol)
    distance = np.hypot(dy, dx)
    ring = np.floor(distance).astype(np.int32)
    angle = np.arctan2(dy, dx)

    # np.lexsort sorts by the last key first, so the effective priority here is:
    # ring -> distance -> angle -> row -> col
    order = np.lexsort((cols, rows, angle, distance, ring))

    rows = rows[order]
    cols = cols[order]
    ring = ring[order]

    coords = tuple((int(r * block), int(c * block)) for r, c in zip(rows, cols))

    headerRegionCount = int(np.count_nonzero(ring < headerRingCount))

    # Reserve the inner rings for header bits; if there are not enough slots, spill over.
    if headerRegionCount >= headerBits:
        headerCoords = coords[:headerRegionCount][:headerBits]
        outerCoords = coords[headerRegionCount:]
        ordered = headerCoords + outerCoords
        usable = len(ordered)
    else:
        # Not enough reserved header slots: the header spills into the outer region,
        # so all coordinates remain usable.
        ordered = coords
        usable = len(coords)

    return ordered, usable, headerRegionCount


def centerRingCoordinates(
        height: int,
        width: int,
        block: int,
        numRequired: int,
        headerBits: int = HEADER_SIZE * 8,
        headerRingCount: int = HEADER_RING_COUNT,
) -> list[tuple[int, int]]:
    """
    Return the first `numRequired` coordinates from the cached center-ring layout.
    """
    ordered, usable, _headerRegionCount = _centerRingLayoutCache(
        height, width, block, headerBits, headerRingCount
    )
    if numRequired > usable:
        raise ValueError(
            f"Required total {numRequired} blocks but only {usable} are usable in center-ring layout"
        )
    return list(ordered[:numRequired])


def robustCapacityBits(
        height: int,
        width: int,
        block: int = 8,
        headerBits: int = HEADER_SIZE * 8,
    ...
    """
    Effective payload capacity under center-ring layout.
    Reuses cached layout metadata instead of recomputing rings in Python.
    """
    _ordered, usable, _headerRegionCount = _centerRingLayoutCache(
        height, width, block, headerBits, headerRingCount
    )
    return int(usable)
...
def embedBits(LL, rawBits, stepSize, midPosition=(3, 3), key: str | None = None):
    ...
    numPayloadBits = int(len(rawBits))
    if key and USE_KEYED_SPATIAL_SHUFFLE:
        maxNumBlock = (embedHeight // 8) * (embedWidth // 8)
    else:
        maxNumBlock = robustCapacityBits(embedHeight, embedWidth, block=8)

    if numPayloadBits > maxNumBlock:
        raise ValueError(
            f"Not enough blocks in the LL band to embed all {numPayloadBits} bits"
    ...
    if key and USE_KEYED_SPATIAL_SHUFFLE:
        ...
        # Robust mode: place header in inner center rings, then place message bits outward.
        coordinates = centerRingCoordinates(
            embedHeight,
            embedWidth,
            block=8,
            numRequired=numPayloadBits,
            headerBits=HEADER_SIZE * 8,
            headerRingCount=HEADER_RING_COUNT,
        )
        coordinatesIter = (
            (y, x, LLband[y: y + 8, x: x + 8]) for (y, x) in coordinates
        )
    ...
    if bitIndex < numPayloadBits:
        raise ValueError(
            f"Successfully embedded only {bitIndex} / {numPayloadBits} bits"
        )
...
def extractDataFromLL(
    ...
    if key and USE_KEYED_SPATIAL_SHUFFLE:
        ...
        # Robust mode: read header from the center rings first, then message bits outward.
        coordinates = centerRingCoordinates(
            validHeight,
            validWidth,
            block=8,
        ...
        # Note: center-ring coordinates are cached by LL size, so repeated images of the same
        # resolution do not rebuild the ordering each time.
        coordinatesIter = (
            (y, x, LLband[y: y + 8, x: x + 8]) for (y, x) in coordinates
        )
...
def capacityBitsForImageBGR(imageBGR: np.ndarray) -> int:
    ...
    validH = (ll_h // 8) * 8
    validW = (ll_w // 8) * 8
    if USE_KEYED_SPATIAL_SHUFFLE:
        return (validH // 8) * (validW // 8)
    return robustCapacityBits(
        validH,
        validW,
        block=8,
        headerBits=HEADER_SIZE * 8,
...
# --- Robust decode search helpers ---

def _rotateImageKeepSize(imageBGR: np.ndarray, angleDeg: float) -> np.ndarray:
    """
    Rotate around image center while keeping the original canvas size.
    Border is reflected to avoid introducing large black regions during decode search.
    """
    h, w = imageBGR.shape[:2]
    center = (w / 2.0, h / 2.0)
    matrix = cv2.getRotationMatrix2D(center, angleDeg, 1.0)
...
def _candidateTransforms(imageBGR: np.ndarray):
    """
    Yield a deterministic set of likely decode candidates.

    Order:
    1) Exact dihedral-style transforms (identity, rotations, flips, diagonal flips)
    2) Small angle wiggles around the four common right-angle orientations

    The decoder stops at the first candidate that passes header validation + CRC.
    """
    # Exact transforms first
    yield "orig", imageBGR
    yield "rot90", cv2.rotate(imageBGR, cv2.ROTATE_90_CLOCKWISE)
    yield "rot180", cv2.rotate(imageBGR, cv2.ROTATE_180)
    yield "rot270", cv2.rotate(imageBGR, cv2.ROTATE_90_COUNTERCLOCKWISE)
    yield "flip_h", cv2.flip(imageBGR, 1)
    yield "flip_v", cv2.flip(imageBGR, 0)
    # Diagonal-equivalent flips via flip+rotation
    yield "flip_h_rot90", cv2.rotate(cv2.flip(imageBGR, 1), cv2.ROTATE_90_CLOCKWISE)
    yield "flip_h_rot270", cv2.rotate(cv2.flip(imageBGR, 1), cv2.ROTATE_90_COUNTERCLOCKWISE)

    # Slight angle wiggles around the most common orientations
    if SEARCH_ANGLE_WIGGLE_DEG > 0 and SEARCH_ANGLE_STEP_DEG > 0:
        deltas = np.arange(
            -float(SEARCH_ANGLE_WIGGLE_DEG),
            float(SEARCH_ANGLE_WIGGLE_DEG) + 1e-6,
            float(SEARCH_ANGLE_STEP_DEG),
            dtype=np.float32,
        )
        baseAngles = (0.0, 90.0, 180.0, 270.0)
        for base in baseAngles:
            for delta in deltas:
                if abs(float(delta)) < 1e-9:
                    continue  # exact angles already yielded above
                angle = float(base + float(delta))
                yield f"rot{int(base)}_{delta:+.0f}deg", _rotateImageKeepSize(imageBGR, angle)


def _decodeFromBGR(imageBGR: np.ndarray, step: float, key: str) -> dict:
    """
    Attempt one decode on a single image orientation.
    Returns a structured result dict without raising for normal decode failures.
    """
    ...
    maxMsgBytes = max(0, (capacityBitsForImageBGR(imageBGR) - (HEADER_SIZE + CRC_SIZE) * 8) // 8)
    if msgLen > maxMsgBytes:
        return {
            "ok": False,
            "stage": "length",
            "detail": {"reason": "unreasonable msgLen", "msgLen": msgLen, "max": maxMsgBytes, "flags": flags},
            "message": None,
        }

    totalBits = (HEADER_SIZE + msgLen + CRC_SIZE) * 8
    payloadBitsWithKey = extractBits(
        imageBGR,
        numBits=totalBits,
        stepSize=float(step),
        key=(key or None),
    ...
    if key:
        payloadBits = XORWhitenBits(payloadBitsWithKey, key=key)
    else:
        payloadBits = payloadBitsWithKey

    decoded = parseMessagePayloadBits(payloadBits)
    return {
        "ok": bool(decoded.get("valid", False)),
        "stage": "payload" if decoded.get("valid", False) else "crc",
        "detail": decoded,
...
def _prettyTransformName(label: str) -> str:
    mapping = {
        "orig": "original",
        "rot90": "rotated 90°",
        "rot180": "rotated 180°",
        ...
        "flip_h_rot90": "diagonal flip (variant 1)",
        "flip_h_rot270": "diagonal flip (variant 2)",
    }
    return mapping.get(label, label)


def robustDecodeSearch(imageBGR: np.ndarray, step: float, key: str, log: logging.Logger) -> dict:
    """
    Try a small deterministic set of candidate transforms and accept the first valid decode.
    This keeps the encoder lightweight while making the decoder more robust.
    """
    lastResult = None
    for label, candidate in _candidateTransforms(imageBGR):
        prettyLabel = _prettyTransformName(label)
        log.info(f"Testing orientation: {prettyLabel}")
        try:
            result = _decodeFromBGR(candidate, step=float(step), key=key)
        ...
        if result.get("ok"):
            ...
            result["transform_pretty"] = prettyLabel
            log.info("✓ Watermark detected")
            log.info(f"Detected transformation: {prettyLabel}")
            return result

        lastResult = result

    # Nothing passed; return the last failure for diagnostics.
    if lastResult is None:
        lastResult = {"ok": False, "stage": "empty", "detail": {"reason": "no candidates tried"}, "message": None}
    lastResult["transform"] = None
    lastResult["transform_pretty"] = None
    return lastResult

def robustDecodeSearch_WIHTOUT_LOG(imageBGR: np.ndarray, step: float, key: str, ) -> dict:
    lastResult = None
    for label, candidate in _candidateTransforms(imageBGR):
        prettyLabel = _prettyTransformName(label)
        print(f"Testing orientation: {prettyLabel}")
        try:
            result = _decodeFromBGR(candidate, step=float(step), key=key)
        except Exception as exc:
            result = {
        ...
        if result.get("ok"):
            ...
            print(f"Detected transformation: {prettyLabel}")
            return result
...
def watermark_camera_image(message, step, key, inputpath, outputpath, quality):
    t_start = time.time()
    ...
    print(f"image_shape={BGR.shape[1]}x{BGR.shape[0]}")
    ...
    embedBGR = watermarkBGR(
        BGR, payloadBitsWithKey, stepSize=float(step), key=(key or None)
    )
    t1 = time.time()
...
def decode_image(inputpath, step, key):
    ...
    if ROBUST_DECODE_SEARCH and not USE_KEYED_SPATIAL_SHUFFLE:
        t_decode_start = time.time()
        print("Watermark extraction started")
        print("")
        if USE_KEYED_SPATIAL_SHUFFLE:
            print("Watermark layout: keyed spatial embedding")
        else:
            print("Watermark layout: center-protected embedding")
        print(f"Header protection rings: {HEADER_RING_COUNT}")
        print("")
        print("Robust decoding: enabled")
        print("Searching for watermark under common image edits...")
        print("")

        result = robustDecodeSearch_WIHTOUT_LOG(BGR, step=float(step), key=key)
        if not result.get("ok"):
            print(f"Decode search failed: {result}")
            return 1

        print("")
        print("Decoded message:")
        print(f'"{result.get("message")}"')
        print(f"Decode time: {(time.time() - t_decode_start):.3f} seconds")
        return 0

    # Fallback: original single-orientation extraction path
    t_decode_start = time.time()
    print("Watermark extraction started")
    print("")
    if USE_KEYED_SPATIAL_SHUFFLE:
    ...
    print("✓ Watermark detected")
    print("Detected transformation: original")
    print("")
    print("Decoded message:")
    print(f'"{decoded.get("message")}"')
    print(f"Decode time: {(time.time() - t_decode_start):.3f} seconds")
    return 0
"""
- EMBED MODE
...
def main() -> int:
    ...
    if args.command == "embed":
        ...
        log.info(f"spatial_shuffle={'on' if USE_KEYED_SPATIAL_SHUFFLE else 'off'}")
        log.info(f"center_ring_layout={'on' if not USE_KEYED_SPATIAL_SHUFFLE else 'off'}")
        log.info(f"header_ring_count={HEADER_RING_COUNT}")
        t_start = time.time()
        ...
        log.info(f"image_shape={BGR.shape[1]}x{BGR.shape[0]}")
    ...
    if args.command == "extract":
        ...
        # Use the robust decode search in the camera implementation as well,
        # while keeping the same outer CLI/adaptor contract.
        if ROBUST_DECODE_SEARCH and not USE_KEYED_SPATIAL_SHUFFLE:
            t_decode_start = time.time()
            log.info("Watermark extraction started")
            log.info("")
            if USE_KEYED_SPATIAL_SHUFFLE:
                log.info("Watermark layout: keyed spatial embedding")
            else:
                log.info("Watermark layout: center-protected embedding")
            log.info(f"Header protection rings: {HEADER_RING_COUNT}")
            log.info("")
            log.info("Robust decoding: enabled")
            log.info("Searching for watermark under common image edits...")
            log.info("")

            result = robustDecodeSearch(BGR, step=float(args.step), key=args.key, log=log)
            if not result.get("ok"):
                log.warning(f"Decode search failed: {result}")
                return 1

            log.info("")
            log.info("Decoded message:")
            log.info(f'"{result.get("message")}"')
            log.info(f"Decode time: {(time.time() - t_decode_start):.3f} seconds")
            return 0
        ...
        log.info("✓ Watermark detected")
        log.info("Detected transformation: original")
        log.info("")
        log.info("Decoded message:")
        log.info(f'"{decoded.get("message")}"')
        log.info(f"Decode time: {(time.time() - t_decode_start):.3f} seconds")
        return 0
